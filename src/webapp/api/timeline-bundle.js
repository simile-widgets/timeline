...
Timeline._Band.prototype._checkAutoWidth=function(){if(!this._timeline.autoWidth){return ;
...
B+=C?this._theme.event.overviewTrack.offset:0;
...
var F={trackOffset:I.track.gap,trackHeight:G,trackGap:I.track.gap,trackIncrement:G+I.track.gap,icon:I.instant.icon,iconWidth:I.instant.iconWidth,iconHeight:I.instant.iconHeight,labelWidth:I.label.width,maxLabelChar:I.label.maxLabelChar,extendLabelDiv:I.label.extendLabelDiv};
var D=this._band.getMinDate();
var B=this._band.getMaxDate();
var J=(this._filterMatcher!=null)?this._filterMatcher:function(K){return true;
};
...
Timeline.OriginalEventPainter.prototype.paintPreciseInstantEvent=function(L,Q,T,R){var U=this._timeline.getDocument();
var J=L.getText();
var F=L.getStart();
var G=Math.round(this._band.dateToPixelOffset(F));
var A=Math.round(G+Q.iconWidth/2);
var C=Math.round(G-Q.iconWidth/2);
var E=this._getLabelDivClassName(L);
var D=this._frc.computeSize(J,E);
var K=A+T.event.label.offsetFromLine;
var H=K+D.width;
var O=H;
var N=this._findFreeTrack(L,O);
var P=Math.round(Q.trackOffset+N*Q.trackIncrement+Q.trackHeight/2-D.height/2);
var B=this._paintEventIcon(L,N,C,Q,T);
var S=this._paintEventLabel(L,J,K,P,D.width,D.height,T,E,R);
var M=this;
var I=function(W,V,X){return M._onClickInstantEvent(B.elmt,V,L);
};
SimileAjax.DOM.registerEvent(B.elmt,"mousedown",I);
SimileAjax.DOM.registerEvent(S.elmt,"mousedown",I);
this._createHighlightDiv(R,B,T,L);
this._eventIdToElmt[L.getID()]=B.elmt;
this._tracks[N]=C;
};
Timeline.OriginalEventPainter.prototype.paintImpreciseInstantEvent=function(N,S,X,U){var Y=this._timeline.getDocument();
var L=N.getText();
var H=N.getStart();
var V=N.getEnd();
var I=Math.round(this._band.dateToPixelOffset(H));
var B=Math.round(this._band.dateToPixelOffset(V));
var A=Math.round(I+S.iconWidth/2);
var D=Math.round(I-S.iconWidth/2);
var G=this._getLabelDivClassName(N);
var F=this._frc.computeSize(L,G);
var M=A+X.event.label.offsetFromLine;
var J=M+F.width;
var Q=Math.max(J,B);
var P=this._findFreeTrack(N,Q);
var R=Math.round(S.trackOffset+P*S.trackIncrement+S.trackHeight/2-F.height/2);
var C=this._paintEventIcon(N,P,D,S,X);
var W=this._paintEventLabel(N,L,M,R,F.width,F.height,X,G,U);
var T=N.getColor();
T=T!=null?T:X.event.instant.impreciseColor;
var E=this._paintEventTape(N,P,I,B,T,X.event.instant.impreciseOpacity,S,X);
var O=this;
var K=function(a,Z,b){return O._onClickInstantEvent(C.elmt,Z,N);
};
SimileAjax.DOM.registerEvent(C.elmt,"mousedown",K);
SimileAjax.DOM.registerEvent(E.elmt,"mousedown",K);
SimileAjax.DOM.registerEvent(W.elmt,"mousedown",K);
this._createHighlightDiv(U,C,X,N);
this._eventIdToElmt[N.getID()]=C.elmt;
this._tracks[P]=D;
};
Timeline.OriginalEventPainter.prototype.paintPreciseDurationEvent=function(K,P,U,R){var V=this._timeline.getDocument();
var I=K.getText();
var E=K.getStart();
var S=K.getEnd();
var F=Math.round(this._band.dateToPixelOffset(E));
var A=Math.round(this._band.dateToPixelOffset(S));
var D=this._getLabelDivClassName(K);
var C=this._frc.computeSize(I,D);
var J=F;
var G=J+C.width;
var N=Math.max(G,A);
var M=this._findFreeTrack(K,N);
var O=Math.round(P.trackOffset+M*P.trackIncrement+U.event.tape.height);
var Q=K.getColor();
Q=Q!=null?Q:U.event.duration.color;
var B=this._paintEventTape(K,M,F,A,Q,100,P,U);
var T=this._paintEventLabel(K,I,J,O,C.width,C.height,U,D,R);
var L=this;
var H=function(X,W,Y){return L._onClickDurationEvent(B.elmt,W,K);
};
SimileAjax.DOM.registerEvent(B.elmt,"mousedown",H);
SimileAjax.DOM.registerEvent(T.elmt,"mousedown",H);
this._createHighlightDiv(R,B,U,K);
this._eventIdToElmt[K.getID()]=B.elmt;
this._tracks[M]=F;
};
Timeline.OriginalEventPainter.prototype.paintImpreciseDurationEvent=function(M,U,Z,W){var a=this._timeline.getDocument();
var K=M.getText();
var G=M.getStart();
var T=M.getLatestStart();
var X=M.getEnd();
var O=M.getEarliestEnd();
var I=Math.round(this._band.dateToPixelOffset(G));
var E=Math.round(this._band.dateToPixelOffset(T));
var A=Math.round(this._band.dateToPixelOffset(X));
var F=Math.round(this._band.dateToPixelOffset(O));
var D=this._getLabelDivClassName(M);
var C=this._frc.computeSize(K,D);
var L=E;
var H=L+C.width;
var Q=Math.max(H,A);
var P=this._findFreeTrack(M,Q);
var R=Math.round(U.trackOffset+P*U.trackIncrement+Z.event.tape.height);
var V=M.getColor();
V=V!=null?V:Z.event.duration.color;
var S=this._paintEventTape(M,P,I,A,Z.event.duration.impreciseColor,Z.event.duration.impreciseOpacity,U,Z);
var B=this._paintEventTape(M,P,E,F,V,100,U,Z);
var Y=this._paintEventLabel(M,K,L,R,C.width,C.height,Z,D,W);
var N=this;
var J=function(c,b,d){return N._onClickDurationEvent(B.elmt,b,M);
};
SimileAjax.DOM.registerEvent(B.elmt,"mousedown",J);
SimileAjax.DOM.registerEvent(Y.elmt,"mousedown",J);
this._createHighlightDiv(W,B,Z,M);
this._eventIdToElmt[M.getID()]=B.elmt;
this._tracks[P]=I;
};
...
Timeline.OriginalEventPainter.prototype._paintEventTape=function(M,B,D,A,G,C,J,I){var F=A-D;
var E=I.event.tape.height;
var K=J.trackOffset+B*J.trackIncrement;
var N=this._timeline.getDocument().createElement("div");
N.className=this._getElClassName("timeline-event-tape",M);
N.id=this._encodeEventElID("tape",M);
N.style.left=D+"px";
N.style.width=F+"px";
N.style.height=E+"px";
N.style.top=K+"px";
if(M._title!=null){N.title=M._title;
}if(G!=null){N.style.backgroundColor=G;
}var L=M.getTapeImage();
var H=M.getTapeRepeat();
H=H!=null?H:"repeat";
if(L!=null){N.style.backgroundImage="url("+L+")";
N.style.backgroundRepeat=H;
}SimileAjax.Graphics.setOpacity(N,C);
this._eventLayer.appendChild(N);
return{left:D,top:K,width:F,height:E,elmt:N};
};
...
Timeline.OriginalEventPainter.prototype._createHighlightDiv=function(A,D,F,B){if(A>=0){var E=this._timeline.getDocument();
var C=this._getHighlightColor(A,F);
var G=E.createElement("div");
G.className=this._getElClassName("timeline-event-highlight",B);
G.id=this._encodeEventElID("highlight",B);
...
L.style.top=H+"px";
L.style.height=I+"px";
if(E!=null){L.style.backgroundColor=E;
...
this.event={track:{height:10,gap:2,autoWidthMargin:1.5},overviewTrack:{offset:20,tickHeight:6,height:2,gap:1,autoWidthMargin:5},tape:{height:4},instant:{icon:Timeline.urlPrefix+"images/dull-blue-circle.png",iconWidth:10,iconHeight:10,impreciseOpacity:20},duration:{impreciseOpacity:20},label:{backgroundOpacity:50,offsetFromLine:3,extendLabelDiv:false},highlightColors:["#FFFF00","#FFC000","#FF0000","#0000FF"],highlightLabelBackground:false,bubble:{width:250,height:125,titleStyler:function(A){A.className="timeline-event-bubble-title";
...
Timeline._Impl.prototype.finishedEventLoading=function(){this._autoWidthChanged(true);
...
Timeline._Impl.prototype.autoSetWidth=function(){this._autoWidthChanged(false);
this._distributeWidths();
};
Timeline._Impl.prototype._autoWidthChanged=function(C){var A=this;
var B=A._starting;
var D=0;
...
H[G]=D+"px";
SimileAjax.jQuery(A._containerDiv).animate(H,A.autoWidthAnimationTime);
}}function F(){var H=0;
var G=A.getPixelWidth();
A._autoResizeTimer=null;
for(var I=0;
I<A._bands.length;
I++){H+=A._bandInfos[I].width;
}if(H>G||C){D=H;
E();
}}if(!A.autoWidth){return ;
}if(B){F();
}else{if(A._autoResizeTimer==null){A._autoResizeTimer=window.setTimeout(F,100);
}}};
