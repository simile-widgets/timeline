...
Timeline.HotZoneEther.prototype.initialize=function(D){this._timeline=D;
this._unit=D.getUnit();
this._zones=[{startTime:Number.NEGATIVE_INFINITY,endTime:Number.POSITIVE_INFINITY,magnify:1}];
var H=this._params;
for(var G=0;
G<H.zones.length;
G++){var C=H.zones[G];
var F=this._unit.parseFromObject(C.start);
var B=this._unit.parseFromObject(C.end);
for(var E=0;
E<this._zones.length&&this._unit.compare(B,F)>0;
E++){var A=this._zones[E];
if(this._unit.compare(F,A.endTime)<0){if(this._unit.compare(F,A.startTime)>0){this._zones.splice(E,0,{startTime:A.startTime,endTime:F,magnify:A.magnify});
E++;
A.startTime=F;
}if(this._unit.compare(B,A.endTime)<0){this._zones.splice(E,0,{startTime:F,endTime:B,magnify:C.magnify*A.magnify});
E++;
A.startTime=B;
F=B;
}else{A.magnify*=C.magnify;
F=A.endTime;
}}}}if("startsOn" in this._params){this._start=this._unit.parseFromObject(this._params.startsOn);
}else{if("endsOn" in this._params){this._start=this._unit.parseFromObject(this._params.endsOn);
this.shiftPixels(-this._timeline.getPixelLength());
...
var H=new Timeline.LinearEther({centersOn:("date" in F)?F.date:new Date(),interval:SimileAjax.DateTime.gregorianUnitLengths[F.intervalUnit],pixelsPerInterval:F.intervalPixels});
...
}}return{width:F.width,eventSource:D,timeZone:("timeZone" in F)?F.timeZone:0,ether:H,etherPainter:C,eventPainter:E};
};
Timeline.createHotZoneBandInfo=function(F){var G=("theme" in F)?F.theme:Timeline.getDefaultTheme();
var D=("eventSource" in F)?F.eventSource:null;
var H=new Timeline.HotZoneEther({centersOn:("date" in F)?F.date:new Date(),interval:SimileAjax.DateTime.gregorianUnitLengths[F.intervalUnit],pixelsPerInterval:F.intervalPixels,zones:F.zones});
...
Timeline._Band=function(B,F,C){this._timeline=B;
this._bandInfo=F;
this._index=C;
this._locale=("locale" in F)?F.locale:Timeline.getDefaultLocale();
this._timeZone=("timeZone" in F)?F.timeZone:0;
this._labeller=("labeller" in F)?F.labeller:(("createLabeller" in B.getUnit())?B.getUnit().createLabeller(this._locale,this._timeZone):new Timeline.GregorianDateLabeller(this._locale,this._timeZone));
...
this._ether=F.ether;
F.ether.initialize(B);
this._etherPainter=F.etherPainter;
F.etherPainter.initialize(this,B);
...
F.eventPainter.initialize(this,B);
