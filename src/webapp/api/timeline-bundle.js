...
this._band.setEventTrackInfo(this._lowerTracks.length+this._upperTracks.length,F.trackIncrement);
};
...
this._lineLayer.style.display="block";
this._eventLayer.style.display="block";
this._band.setEventTrackInfo(this._tracks.length,F.trackIncrement);
};
...
Timeline._Impl.prototype.autoSetWidths=function(){var E=0;
var D=false;
for(var C=0;
C<this._bands.length;
C++){var F=this._bands[C];
var B=this._bandInfos[C];
var A=F.desiredWidth();
if(A!=null){B.width=A;
E+=A;
}else{D=true;
}}return D?null:E;
...
Timeline._Band.prototype.setEventTrackInfo=function(A,B){this._eventTracksNeeded=Math.max(A,this._eventTracksNeeded);
this._eventTrackIncrement=B;
};
Timeline._Band.prototype.desiredWidth=function(){var A=this._eventPainter.getType()=="overview"?this._theme.event.overviewTrack.autoWidthMargin:this._theme.event.track.autoWidthMargin;
return Math.ceil((this._eventTracksNeeded+A)*this._eventTrackIncrement);
};
