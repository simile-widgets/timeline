...
/* band.js */
Timeline._Band=function(B,H,C){if(B.autoWidth&&typeof H.width=="string"){H.width=H.width.indexOf("%")>-1?0:parseInt(H.width);
}this._timeline=B;
this._bandInfo=H;
this._index=C;
this._locale=("locale" in H)?H.locale:Timeline.getDefaultLocale();
this._timeZone=("timeZone" in H)?H.timeZone:0;
this._labeller=("labeller" in H)?H.labeller:(("createLabeller" in B.getUnit())?B.getUnit().createLabeller(this._locale,this._timeZone):new Timeline.GregorianDateLabeller(this._locale,this._timeZone));
this._theme=H.theme;
...
var F=this._timeline.getDocument().createElement("div");
F.className="timeline-band-input";
this._timeline.addDiv(F);
this._keyboardInput=document.createElement("input");
this._keyboardInput.type="text";
F.appendChild(this._keyboardInput);
...
var G=this._theme!=null?this._theme.mouseWheel:"scroll";
if(G==="zoom"||G==="scroll"||this._zoomSteps){if(SimileAjax.Platform.browser.isFirefox){SimileAjax.DOM.registerEventWithObject(this._div,"DOMMouseScroll",this,"_onMouseScroll");
}else{SimileAjax.DOM.registerEventWithObject(this._div,"mousewheel",this,"_onMouseScroll");
}}this._innerDiv=this._timeline.getDocument().createElement("div");
this._innerDiv.className="timeline-band-inner";
this._div.appendChild(this._innerDiv);
this._ether=H.ether;
H.ether.initialize(this,B);
this._etherPainter=H.etherPainter;
H.etherPainter.initialize(this,B);
...
H.eventPainter.initialize(this,B);
this._decorators=("decorators" in H)?H.decorators:[];
for(var E=0;
E<this._decorators.length;
E++){this._decorators[E].initialize(this,B);
...
var D=this._scrollBar.firstChild;
if(SimileAjax.Platform.browser.isIE){D.style.cursor="move";
}else{D.style.cursor="-moz-grab";
}SimileAjax.DOM.registerEventWithObject(D,"mousedown",this,"_onScrollBarMouseDown");
}};
...
Timeline._Band.prototype._onMouseMove=function(F,A,G){if(this._dragging||this._orthogonalDragging){var D=A.clientX-this._dragX;
var C=A.clientY-this._dragY;
this._dragX=A.clientX;
this._dragY=A.clientY;
}if(this._dragging){if(this._timeline.isHorizontal()){this._moveEther(D,C);
}else{this._moveEther(C,D);
}}else{if(this._orthogonalDragging){var B=this.getViewWidth();
var E=this._scrollBar.firstChild;
if(this._timeline.isHorizontal()){this._moveEther(0,-C*B/E.offsetHeight);
}else{this._moveEther(0,-D*B/E.offsetWidth);
}}else{return ;
}}this._positionHighlight();
this._showScrollbar();
return this._cancelEvent(A);
...
Timeline._Band.prototype._positionHighlight=function(){if(this._syncWithBand){var F=this._syncWithBand.getMinVisibleDate();
var H=this._syncWithBand.getMaxVisibleDate();
if(this._highlight){var G=0;
var B=1;
var A=this._syncWithBand.getEventPainter();
if("supportsOrthogonalScrolling" in A&&A.supportsOrthogonalScrolling()){var D=A.getOrthogonalExtent();
var C=this._syncWithBand.getViewWidth();
var E=Math.max(C,D);
B=C/E;
G=-this._syncWithBand.getViewOrthogonalOffset()/E;
}this._etherPainter.setHighlight(F,H,G,B);
}}};
...
}var D=this._eventPainter.getOrthogonalExtent();
var C=this.getViewWidth();
var G=Math.max(C,D);
var E=(C/G);
var B=Math.round(C*E)+"px";
var H=Math.round(-this._viewOrthogonalOffset*E)+"px";
var F=12;
var A=this._scrollBar.firstChild;
...
this._scrollBar.style.width=F+"px";
A.style.top=H;
A.style.height=B;
...
this._scrollBar.style.height=F+"px";
A.style.left=H;
A.style.width=B;
}if(E>=1&&this._viewOrthogonalOffset==0){this._scrollBar.style.display="none";
...
Timeline.CompactEventPainter.prototype.paint=function(){var D=this._band.getEventSource();
if(D==null){return ;
}this._eventIdToElmt={};
this._prepareForPainting();
var L=this._computeMetrics();
var G=this._band.getMinDate();
var B=this._band.getMaxDate();
var O=(this._filterMatcher!=null)?this._filterMatcher:function(Q){return true;
};
var A=(this._highlightMatcher!=null)?this._highlightMatcher:function(Q){return -1;
};
var J=D.getEventIterator(G,B);
...
if(C||M){var P=[];
var E=null;
while(J.hasNext()){var N=J.next();
if(O(N)){if(!N.isInstant()||N.isImprecise()){this.paintEvent(N,L,this._params.theme,A(N));
}else{if(E!=null&&E.getStart().getTime()==N.getStart().getTime()){P[P.length-1].push(N);
}else{P.push([N]);
E=N;
}}}}for(var H=0;
H<P.length;
H++){var K=P[H];
if(K.length==1){this.paintEvent(K[0],L,this._params.theme,A(N));
}else{var I=-1;
for(var F=0;
I<0&&F<K.length;
F++){I=A(K[F]);
}if(M){this.paintStackedPreciseInstantEvents(K,L,this._params.theme,I);
}else{this.paintCompositePreciseInstantEvents(K,L,this._params.theme,I);
}}}}else{while(J.hasNext()){var N=J.next();
if(O(N)){this.paintEvent(N,L,this._params.theme,A(N));
...
var E={start:I.getStart(),end:I.getEnd(),latestStart:I.getLatestStart(),earliestEnd:I.getEarliestEnd(),color:I.getColor()||I.getTextColor(),isInstant:true};
var F={url:I.getIcon()};
...
var E={start:I.getStart(),end:I.getEnd(),color:I.getColor()||I.getTextColor(),isInstant:false};
var F={url:I.getIcon()};
if(F.url==null){F=null;
}else{F.width=I.getProperty("iconWidth")||G.customIconWidth;
F.height=I.getProperty("iconHeight")||G.customIconHeight;
...
var E={start:I.getStart(),end:I.getEnd(),latestStart:I.getLatestStart(),earliestEnd:I.getEarliestEnd(),color:I.getColor()||I.getTextColor(),isInstant:false};
var F={url:I.getIcon()};
if(F.url==null){F=null;
}else{F.width=I.getProperty("iconWidth")||G.customIconWidth;
F.height=I.getProperty("iconHeight")||G.customIconHeight;
...
Timeline.GregorianEtherPainter.prototype.setHighlight=function(B,D,C,A){this._highlight.position(B,D,C,A);
};
...
this.position=function(J,M,K,G){K=K||0;
G=G||1;
this._createHighlightDiv();
var L=Math.round(E.dateToPixelOffset(J));
var I=Math.round(E.dateToPixelOffset(M));
var F=Math.max(I-L,3);
var H=E.getViewWidth()-4;
if(A){this._highlightDiv.style.left=L+"px";
this._highlightDiv.style.width=F+"px";
this._highlightDiv.style.top=Math.round(K*H)+"px";
this._highlightDiv.style.height=Math.round(G*H)+"px";
}else{this._highlightDiv.style.top=L+"px";
this._highlightDiv.style.height=F+"px";
this._highlightDiv.style.left=Math.round(K*H)+"px";
this._highlightDiv.style.width=Math.round(G*H)+"px";
}};
};
...
Timeline.DefaultEventSource.prototype.loadJSON=function(G,A){var C=this._getBaseURL(A);
var I=false;
...
var I=("decorators" in F)?F.decorators:[];
...
var J={showText:("showEventText" in F)?F.showEventText:true,theme:G};
if("eventPainterParams" in F){for(var A in F.eventPainterParams){J[A]=F.eventPainterParams[A];
}}if("trackHeight" in F){J.trackHeight=F.trackHeight;
}if("trackGap" in F){J.trackGap=F.trackGap;
}var B=("overview" in F&&F.overview)?"overview":("layout" in F?F.layout:"original");
var E;
if("eventPainter" in F){E=new F.eventPainter(J);
}else{switch(B){case"overview":E=new Timeline.OverviewEventPainter(J);
break;
case"detailed":E=new Timeline.DetailedEventPainter(J);
break;
default:E=new Timeline.OriginalEventPainter(J);
}}return{width:F.width,eventSource:D,timeZone:("timeZone" in F)?F.timeZone:0,ether:H,etherPainter:C,eventPainter:E,theme:G,decorators:I,zoomIndex:("zoomIndex" in F)?F.zoomIndex:0,zoomSteps:("zoomSteps" in F)?F.zoomSteps:null};
};
