...
F<G.events.length;
F++){var A=G.events[F];
var K=new Timeline.DefaultEventSource.Event(("id" in A)?A.id:undefined,D(A.start),D(A.end),D(A.latestStart),D(A.earliestEnd),A.isDuration||false,A.title,A.description,this._resolveRelativeURL(A.image,C),this._resolveRelativeURL(A.link,C),this._resolveRelativeURL(A.icon,C),A.color,A.textColor,A.hoverText,A.classname,A.barImage,A.barRepeat);
K._obj=A;
...
}var L=new Timeline.DefaultEventSource.Event(C["id"],F(C["start"]),F(C["end"]),F(C["latestStart"]),F(C["earliestEnd"]),C["isDuration"]!="true",C["title"],C["description"],this._resolveRelativeURL(C["image"],D),this._resolveRelativeURL(C["link"],D),this._resolveRelativeURL(C["icon"],D),C["color"],C["textColor"],C["hoverText"],C["classname"],C["barImage"],C["barRepeat"]);
L._bindings=C;
...
Timeline.DefaultEventSource.Event=function(D,C,I,L,E,B,Q,P,F,N,M,H,O,G,J,K,A){D=(D)?D.trim():"";
this._id=D.length>0?D:("e"+Math.floor(Math.random()*1000000));
this._instant=B||(I==null);
this._start=C;
this._end=(I!=null)?I:C;
this._latestStart=(L!=null)?L:(B?this._end:this._start);
this._earliestEnd=(E!=null)?E:(B?this._start:this._end);
this._text=SimileAjax.HTML.deEntify(Q);
this._description=SimileAjax.HTML.deEntify(P);
this._image=(F!=null&&F!="")?F:null;
this._link=(N!=null&&N!="")?N:null;
this._title=(G!=null)?G:null;
this._icon=(M!=null&&M!="")?M:null;
this._color=(H!=null&&H!="")?H:null;
this._textColor=(O!=null&&O!="")?O:null;
this._classname=(J!=null&&J!="")?J:null;
this._barImage=(K!=null&&K!="")?K:null;
this._barRepeat=(A!=null&&A!="")?A:null;
this._wikiURL=null;
