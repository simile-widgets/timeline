...
var A=this._timeline.getDocument().createElement("div");
A.className=this._getElClassName("timeline-event-icon",K);
...
Timeline.OverviewEventPainter.prototype.paintInstantEvent=function(I,H,E,B){var F=I.getStart();
var C=Math.round(this._band.dateToPixelOffset(F));
var D=I.getColor(),A=I.getClassName();
if(A){D=null;
}else{D=D!=null?D:E.event.duration.color;
}var G=this._paintEventTick(I,C,D,100,H,E);
this._createHighlightDiv(B,G,E);
};
Timeline.OverviewEventPainter.prototype.paintDurationEvent=function(L,K,H,C){var B=L.getLatestStart();
var I=L.getEarliestEnd();
var J=Math.round(this._band.dateToPixelOffset(B));
var D=Math.round(this._band.dateToPixelOffset(I));
var G=0;
for(;
G<this._tracks.length;
G++){if(D<this._tracks[G]){break;
}}this._tracks[G]=D;
var F=L.getColor(),A=L.getClassName();
if(A){F=null;
}else{F=F!=null?F:H.event.duration.color;
}var E=this._paintEventTape(L,G,J,D,F,100,K,H,A);
this._createHighlightDiv(C,E,H);
};
Timeline.OverviewEventPainter.prototype._paintEventTape=function(K,C,E,L,F,D,H,G,A){var I=H.trackOffset+C*H.trackIncrement;
var B=L-E;
var J=H.trackHeight;
var M=this._timeline.getDocument().createElement("div");
M.className="timeline-small-event-tape";
if(A){M.className+=" small-"+A;
}M.style.left=E+"px";
M.style.width=B+"px";
M.style.top=I+"px";
M.style.height=J+"px";
if(F){M.style.backgroundColor=F;
}if(D<100){SimileAjax.Graphics.setOpacity(M,D);
}this._eventLayer.appendChild(M);
return{left:E,top:I,width:B,height:J,elmt:M};
};
Timeline.OverviewEventPainter.prototype._paintEventTick=function(J,D,E,C,G,F){var I=F.event.overviewTrack.tickHeight;
...
K.style.left=D+"px";
K.style.top=H+"px";
var A=J.getClassName();
if(A){K.className+=" small-"+A;
}if(C<100){SimileAjax.Graphics.setOpacity(K,C);
}this._eventLayer.appendChild(K);
return{left:D,top:H,width:B,height:I,elmt:K};
};
